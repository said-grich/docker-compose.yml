jQuery(document).ready(function(){
		...
		items.each(function(){
			...
			selectOptions.on('click', function(event) { 
				...
				if($(event.target).is('li')) {
					...
					selected.removeClass('selected-1 selected-2 selected-3 selected-4 selected-5 selected-6 selected-7 selected-8').addClass('selected-'+index);
					// if color has been changed, update the visible product image 
					selected.hasClass('color') && updateSlider(selected, index-1);
					selected.hasClass('size') && updateSlider(selected);
				}
			});
	...
	function updateSlider(actual, index) {
		var slider = actual.parent('.cd-customization').prev('ul'),
			slides = slider.children('li'),
			btn = actual.parent('.cd-customization').children('.add-to-cart'),
			pic = slides.eq(index).children('a').children('img').attr("src"),
			color = actual.parent('.cd-customization').children('.color').find('li.active').prop("textContent"),
			size = actual.parent('.cd-customization').children('.size').find('li.active').prop("textContent");

		$(btn).attr("data-pic", pic);
