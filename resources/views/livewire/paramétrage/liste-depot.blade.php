            ...
            <div class="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
                <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
                    ...
                    <tr>--}}
                        ...
                        {{--<th scope="col" class="px-6 py-3 text-left text-xs font-medium text-indigo-700 uppercase tracking-wider">Code depot</th>
                        ...
                        <th scope="col"class="px-6 py-3 text-left text-xs font-medium text-indigo-700 uppercase tracking-wider">Mode de stockage</th>
                        <!--<th scope="col" class="px-6 py-3 text-left text-xs font-medium text-indigo-700 uppercase tracking-wider">Mobile</th>-->
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">--}}
                        <th class="pl-0" wire:click="sortBy('code')" style="cursor: pointer;">Code depot @include('layouts.partials._sort-icon',['field'=>'code'])</th>
                        <th class="pl-0" wire:click="sortBy('name')" style="cursor: pointer;">Nom @include('layouts.partials._sort-icon',['field'=>'name'])</th>
                        {{--<th class="pl-0" wire:click="sortBy('siteId')" style="cursor: pointer;">Site </th>--}}
                        ...
                        <th class="pl-0" wire:click="sortBy('mode_stockage')" style="cursor: pointer;">Mode de stockage @include('layouts.partials._sort-icon',['field'=>'mode_stockage'])</th>
                        {{--<th class="pl-0" wire:click="" style="cursor: pointer;">Mobile </th>--}}
                ...
                <tbody>
                    @foreach ($depots as $item)
                        <tr @if($loop->even)class="bg-grey"@endif>
                            ...
                            <td class="pl-0">
                                <a href="#" class="text-dark-75 font-weight-bolder text-hover-primary font-size-lg">{{ $item->mode_stockage }}</a>
                            </td>
                           ...
                           {{--<td class="px-6 py-4 whitespace-nowrap text-sm">{{ $item->code }}</td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm">{{ $item->mode_stockage}}</td>
                            <!--<td class="px-6 py-4 whitespace-nowrap text-sm">{{ $item->phone }}</td>-->
                            <td class="w-15">--}}
                                <td class="pr-0 text-right">

                                <a   href="{{ route('edit-depot', ['ida' => $item->id]) }}" class="btn btn-icon btn-light btn-hover-primary btn-sm mx-3" >
                            ...
                            <td class="w-15">
                                ...
                                <svg class="w-5 h-5 cursor-pointer fill-current text-red-500" wire:click="deleteDepot('{{$item->id}}')"  xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              {{ $depots->links('layouts.partials.custom-pagination') }}
            </div></div></div></div>
