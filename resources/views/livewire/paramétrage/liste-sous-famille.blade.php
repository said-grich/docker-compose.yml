    ...
    <table class="table table-head-custom table-vertical-center" id="kt_advance_table_widget_4">
        <thead>
            <tr class="text-left">
                ...
                <th class="pl-0" wire:click="sortBy('code')" style="cursor: pointer;">Code famille @include('layouts.partials._sort-icon',['field'=>'code'])</th>
                <th class="pl-0" wire:click="sortBy('name')" style="cursor: pointer;">Sous-famille @include('layouts.partials._sort-icon',['field'=>'name'])</th>
                <th class="pr-0 text-right" style="min-width: 160px">Actions</th>
            </tr>
        </thead>
        <tbody>
            @foreach ($sousfamille as $item)
                ...
                <td class="pr-0 text-right">
                    <a  href="{{route('edit-sous-famille', ['ida' => $item->id])}}" class="btn btn-icon btn-light btn-hover-primary btn-sm mx-3">
                    ...
                    <a href="#" class="btn btn-icon btn-light btn-hover-primary btn-sm" wire:click="deleteSousFamille('{{$item->id}}')">
                        <span class="svg-icon svg-icon-md svg-icon-primary">
                            {{--begin::Svg Icon--}}
                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1">
                                <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
...
{{ $sousfamille->links('layouts.partials.custom-pagination') }}
</div>
...
<div class="-my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
            <div class="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
                <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
                    ...
                    <tr>
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-indigo-700 uppercase tracking-wider">Code famille</th>
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-indigo-700 uppercase tracking-wider">Sous-famille</th>
                    ...
                    @foreach ($list as $item)
                        <tr @if($loop->even)class="bg-grey"@endif>
                            ...
                            <td class="w-15">
                                <a   href="{{ route('edit-sous-famille', ['ida' => $item->id]) }}">
                            ...
                            <td class="w-15">
                                <svg class="w-5 h-5 cursor-pointer fill-current text-red-500" wire:click="deleteSousFamille('{{$item->id}}')"  xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
