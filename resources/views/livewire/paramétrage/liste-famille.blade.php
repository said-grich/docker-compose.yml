    ...
    <table class="table table-head-custom table-vertical-center" id="kt_advance_table_widget_4">
        <thead>
            <tr class="text-left">
                ...
                <th class="pl-0" wire:click="sortBy('famille')" style="cursor: pointer;">Famille @include('layouts.partials._sort-icon',['field'=>'famille'])</th>
        ...
        <tbody>
            @foreach ($famille as $item)
                <tr @if($loop->even)class="bg-grey"@endif>
                    ...
                    <td class="pl-0">
                        <a href="#" class="text-dark-75 font-weight-bolder text-hover-primary font-size-lg">{{ $item->famille }}</a>
                    </td>
                    <td class="pr-0 text-right">
                        ...
                        <a href="#" class="btn btn-icon btn-light btn-hover-primary btn-sm" wire:click="deleteFamille('{{$item->id}}')">
                            <span class="svg-icon svg-icon-md svg-icon-primary">
                                {{--begin::Svg Icon--}}
                                <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1">
                                    <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
        ...
        {{ $famille->links('layouts.partials.custom-pagination') }}
    </div>
                ...
                <table class="mb-3 min-w-full divide-y divide-gray-200">
                    ...
                    @foreach ($list as $item)
                        <tr @if($loop->even)class="bg-grey"@endif>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm">{{ $item->famille }}</td>
                            <td class="w-15">
                                <a   href="{{ route('edit-famille', ['ida' => $item->id]) }}">
                            ...
                            <td class="w-15">
                                <svg class="w-5 h-5 cursor-pointer fill-current text-red-500" wire:click="deleteFamille('{{$item->id}}')"  xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
