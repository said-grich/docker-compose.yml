    ...
    <table class="table table-head-custom table-vertical-center" id="kt_advance_table_widget_4">
        <thead>
            <tr class="text-left">
                ...
                <th class="pl-0" wire:click="sortBy('nom')" style="cursor: pointer;">Mode de préparation @include('layouts.partials._sort-icon',['field'=>'nom'])</th>
            ...
            @foreach ($items as $item)
                <tr @if($loop->even)class="bg-grey"@endif>
                    ...
                    <td class="pr-0 text-right">
                        ...
                        <a href="#" class="btn btn-icon btn-light btn-hover-primary btn-sm" wire:click="deletePreparation('{{$item->id}}')">
                            <span class="svg-icon svg-icon-md svg-icon-primary">
                                {{--begin::Svg Icon--}}
                                <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1">
                                    <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
            ...
            <div class="modal-dialog modal-dialog-centered modal-xl" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel">{{ __('Modification Sous Mode Prepartion') }}</h5>
                    ...
                    <div wire:ignore class="modal-body">
                        <form id="edit-form" class="form row">
                            ...
                            <div class="input-group input-group-prepend">
                                ...
                                <select class="form-control" wire:model="mode_preparation">
                                    <option>{{ __('Mode Préparation') }}</option>
                                    @foreach ($list_mode_preparations as $mode)
                                            <option value="{{$mode->id}}" @if($mode_preparation == $mode->id) {{'selected'}} @endif>{{$mode->nom}}</option>
                                        @endforeach
                                </select>
                            </div>
                        ...
                        <div class="modal-footer">
                            ...
                            <button type="submit" wire:click.prevent="editPreparation" class="btn btn-primary font-weight-bold" form="edit-form" >{{ __('Enregistrer') }}</button>
                        </div>
                    </div>
