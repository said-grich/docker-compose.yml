<div class="min-w-7xl mx-auto py-10 sm:px-6 lg:px-8">
    <x-slot name="header">
        <h2 class="text-xl font-semibold leading-tight text-gray-800">
            {{ __('Modification bon de livraison') }}
        </h2>
    </x-slot>
    ...
    <form wire:submit.prevent="transfertBonCommande">
        ...
        <div class="grid grid-cols-7 gap-4 p-4 mb-8">
            ...
            <label class="block">
                ...
                <input wire:model.defer="date"  type="date" class="block w-full mt-1 form-input" id="date"
                    autocomplete="date">
            </label>
            <label class="block">
                <span class="text-gray">Bon livraison ref.</span>
                <input type="text" wire:model.lazy="refBonLivraison" class="block w-full mt-1 form-input" placeholder="">
            </label>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="divide-y divide-gray-200 bg-gray">
                    ...
                    @for ($line = 0; $line < $lines_count; $line++)
                        <tr>
                            <td class="border p-2 border border-gray-200" x-data="{isOpen{{ $line }}:false}">
                                ...
                                <ul x-show="isOpen{{ $line }}" @click.away="isOpen{{ $line }}=false"
                                    class="absolute z-10 cursor-pointer bg-orange-400 text-white py-1 w-1/4">
                                    @if (!empty($articles))
