...
class EditBonLivraison extends Component
{
    use WithPagination;
    ...
    public function mount()
    {
        ...
        $list = BonLivraisonLine::where('bon_livraison_ref', $this->ida)->get();

        $this->lines_count = count($list);
        $this->ref = $list[0]->bon_livraison_ref;
        $this->date = $list[0]->bonLivraison->date;
        $this->clientId = $list[0]->bonLivraison->client_id;
        $this->commercialId = $list[0]->bonLivraison->commercial_id;
        $this->siteId = $list[0]->bonLivraison->site_id;

        $this->totalMt = $list[0]->bonLivraison->totalMt;
        $this->totalTtc = $list[0]->bonLivraison->totalTtc;
        $this->totalTva = $list[0]->bonLivraison->totalTva;
    ...
    public function editBonLivraison()
    {
        ...
        DB::transaction(function () {
            $item = BonLivraison::where('ref', $this->ref)->first();
            $item->date = $this->date;
            ...
            //BonLivraisonsLine::where('bon_livraison_ref', $this->ref)->update(['mod' => 1]);
            BonLivraisonLine::where('bon_livraison_ref', $this->ref)->delete();

            foreach ($this->code as $key => $value) {

                BonLivraisonLine::create(['bon_livraison_ref' => $this->ref, 'article_id' => $this->articleId[$key], 'libelle_article' => $this->libelle[$key], 'qte' => $this->qte[$key], 'tva' => $this->tva[$key], 'prix' => $this->prix[$key], 'montant' => $this->montant[$key]]);
            }
        });
        return redirect()->route('create-bon-livraison');
    }
    ...
    public function render()
    {
        ...
        return view('livewire.vente.edit-bon-livraison', ['list_clients' => $list_clients, 'liste_commercials' => $liste_commercials, 'list_sites' => $list_sites, 'articles' => $this->articles, 'lines_count' => $this->lines_count]);
    }
}
