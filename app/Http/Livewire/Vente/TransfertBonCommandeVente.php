...
class TransfertBonCommandeVente extends Component
{
    public $refBonLivraison;
    public $articles = [];
    public $libelle = [];
    public $tva = [];
    public $qte = [];
    ...
    public function mount()
    {
        ...
        $list = BonCommandeVenteLine::where('bon_commande_ref', $this->ref)->get();
        //dd($list);
    ...
    public function transfertBonCommande()
    {
        ...
        DB::transaction(function () {
            ...
            //BonCommandeVenteLine::where('bon_livraison_ref', $this->ref)->delete();
            BonCommandeVente::where('ref', $this->ref)->update(['validation' => 1]);
            foreach ($this->code as $key => $value) {

                BonLivraisonLine::create(['bon_livraison_ref' => $this->refBonLivraison, 'article_id' => $this->articleId[$key], 'libelle_article' => $this->libelle[$key], 'qte' => $this->qte[$key], 'tva' => $this->tva[$key], 'prix' => $this->prix[$key], 'montant' => $this->montant[$key]]);
    ...
    public function render()
    {
        ...
        return view('livewire.vente.transfert-bon-commande-vente', ['list_clients' => $list_clients, 'liste_commercials' => $liste_commercials, 'list_sites' => $list_sites, 'list_depots' => $list_depots, 'articles' => $this->articles, 'lines_count' => $this->lines_count]);
    }
}
